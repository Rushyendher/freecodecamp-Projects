...
function displayStrems(streams){
	for(var i = 0;i<streams.length;i++){
		var streamObj = streams[i];
		var streamName = "No StreamName";
		var streamStatatus = "No Status";
		var imgURL = "https://dummyimage.com/50x50/ecf0e7/5c5457.jpg";
		
		console.log(streamObj);
		
		if(streamObj.stream){
			streamName = streamObj.stream.display_name;
			streamStatatus = streamObj.stream.status;
			imgURL = streamObj.stream.logo;
		}
		
		var html = "<div class='row'>"+
						"<div class='col-md-3'>"+
							"<img src='"+ imgURL + "' class='img-rounded' width='50' height='50'/>"+
						"</div>"+
						"<div class='col-md-3'>"+
							"<h4>" + streamName + "</h4>"+
						"</div>"+
						"<div class='col-md-6'>"+
							"<h4>" + streamStatatus + "</h4>"+
						"</div>"+						
					"</div> <br/>"
		$("#streams").append(html)
	}
...
$(document).ready(function(){
	...
	for(var i = 0;i<json.length;i++){
		var streamTemp = {};
		var streamObj = json[i];
		streamTemp.links = streamObj._links;
		streamTemp.stream = streamObj.stream;
		
		if(streamTemp.stream)
			online.push(streamTemp);
		else
			offline.push(streamTemp);
		
		allStreams.push(streamTemp);
	}
	console.log(allStreams);
	$("#all").on("click",function(){
		$("#streams").html('')
		displayStrems(allStreams);
	});
	$("#online").on("click",function(){
		$("#streams").html('')
		displayStrems(online);
	});	
	$("#offline").on("click",function(){
		$("#streams").html('')
		displayStrems(offline);
	});	
